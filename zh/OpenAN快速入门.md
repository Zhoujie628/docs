...
## 项目架构
OpenAN 设置了 6 个项目组模块（SIG），构成了完整的智能体协作框架：
- SIG 1 A2A-T（Agent2Agent-telecom）SDK：提供Agent交互之间的意图模板，校验消息合法性，提供消息压缩机制，提升交互效率。
- SIG 2 注册与编排：提供AgentCard的注册管理能力，按意图匹配Skill，提供多个功能侧agent流程编排能力。
- SIG 3 场景化实践：运营商/厂商分享场景化实践（Solution package），其他运营商可以获取参考，改造运行。
- SIG4 执行引擎：负责对上解析上层Agent意图，按照workflow执行，或自生成规划步骤，按需调用通信技能库。
- SIG5 通信策略演进：负责对模型生成的步骤进行评价，牵引模型提示框架持续改进。
- SIG6 通信技能与组件库：负责各种场景下的技能、组件、知识等，运营商、厂商分享贡献。
> **说明**：openAN首次开源发布SIG 1 A2A-T（Agent2Agent-telecom）SDK、SIG 2 注册与编排、SIG 3 场景化实践，其他SIG后续版本持续演进。
